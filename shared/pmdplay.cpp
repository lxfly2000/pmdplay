...
int PMDPlay::Init(TCHAR* param)
{
	_pObj = this;
	setlocale(LC_ALL, "");
	player.Init(CHANNELS, SAMPLE_RATE, BYTES_PER_VAR, 0);
	char rhydir[] = ".";
	//加载节奏声音
	if (!player.LoadRhythmFromDirectory(rhydir))
	{
		const int files_id[] = {
...
void PMDPlay::OnDraw()
{
	...
	if (showVoiceAndVolume)
	{
		...
		{
			if (player.GetKeysState()[i] == -1)
				strcpyDx(drawbuf, TEXT("  -"));
			else
				sprintfDx(drawbuf, TEXT("%3d"), player.GetKeysState()[i]);
			DrawFormatString(keydisp_x, keydisp_y + keydisp_onechannel_h * i, 0x00FFFFFF, TEXT("%s %3d %3d"),
				drawbuf, player.GetKeyVoice()[i], player.GetKeyVolume()[i]);
		}
	}
	ViewMemDraw();
	ScreenFlip();
...
int pmdplayMain(LPWSTR lpCmdLine)
{
